"""Fine-tune NSTP from WikiText-2 checkpoint on FineWeb-Edu 100M tokens"""
...
SEQ, VS, DM, NL, NH = 128, 50257, 320, 3, 4
HSA_DIM, NE, TK, DFF = 2048, 4, 2, 768
RTR, ETR, DROP = [1,4,4,1], [1,4,4,4,1], 0.1
BATCH = 32; MAX_STEPS = 30000; EVAL_EVERY = 1000
LR, CLIP = 1e-4, 1.0  # Lower LR for fine-tuning
CKPT = 'C:/Users/user/AppData/Local/Temp/nstp-v2/models_scaled/finetune_best.pt'
...
print(f"Train batches/epoch: {len(train_ds)//BATCH}")

class VH:
    @staticmethod
    def bind(h,pos,hd):
        freq=torch.fft.rfft(h,dim=-1); n=freq.shape[-1]
        f=torch.arange(n,device=h.device,dtype=torch.float)
    ...
    @staticmethod
    def unbind(M,pos,hd):
        B,S=pos.shape; Me=M.unsqueeze(1).expand(-1,S,-1)
        freq=torch.fft.rfft(Me,dim=-1); n=freq.shape[-1]
        f=torch.arange(n,device=M.device,dtype=torch.float)
        angle=2*math.pi*f*(-pos.float()).unsqueeze(-1)/hd
        rot=torch.complex(torch.cos(angle),torch.sin(angle))
        return torch.fft.irfft(freq*rot,n=hd,dim=-1)
...
class NSTP(nn.Module):
    def __init__(self):
        super().__init__()
        self.embed=nn.Embedding(VS,DM)
        self.blocks=nn.ModuleList([Block(DM,HSA_DIM,NH,NE,TK,DFF,RTR,ETR,DROP) for _ in range(NL)])
...
model=NSTP().to(DEVICE)

# Load WikiText-2 pretrained weights
...
if os.path.exists(WK_CKPT):
    ...
    print(f"Loaded WikiText-2 weights: {len(sd)} keys, missing={len(missing)}, unexpected={len(unexpected)}")
    if missing: print(f"  Missing: {missing[:5]}...")
else:
    print(f"WARNING: {WK_CKPT} not found, training from scratch")
