"""Train NSTP on FineWeb-Edu 100M tokens (SEQ=128, cosine LR, dropout=0.1)"""
...
BATCH = 32; MAX_STEPS = 60000; EVAL_EVERY = 2000
LR, CLIP = 3e-4, 1.0
CKPT = 'C:/Users/user/AppData/Local/Temp/nstp-v2/models_scaled/fwenbu_best.pt'
...
class VH:
    @staticmethod
    def bind(h,pos,hd):
        freq=torch.fft.rfft(h,dim=-1); n=freq.shape[-1]; f=torch.arange(n,device=h.device,dtype=torch.float)
    ...
    @staticmethod
    def unbind(M,pos,hd):
        B,S=pos.shape; Me=M.unsqueeze(1).expand(-1,S,-1); freq=torch.fft.rfft(Me,dim=-1)
