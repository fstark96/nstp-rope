...
class NSTPConfig:
    ...
    # Optimization
    max_seq_len: int = 1_000_000  # Effectively unlimited with HSA
...
class NSTPBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            return_aux_loss: Whether to return MoE aux loss
        
        Returns:
            output: [batch, seq_len, d_model]
            moe_aux_loss: Optional MoE load balancing loss
            hsa_context: Optional HSA context for analysis
        """
...
class NSTPModel(nn.Module):
    ...
    Architecture:
    - TT Embedding (optional) or standard embedding
    ...
    def __init__(self, config: NSTPConfig):
        ...
        if config.use_tt_embedding:
            # For TT embedding, we need a separate head or tied projection
        ...
        if not config.use_tt_embedding:
            ...
            nn.init.normal_(self.pos_embedding.weight, std=config.init_std)
    ...
    def generate(
        ...
        Note: With HSA, we can efficiently handle very long contexts
        because context accumulation is O(1) per token.
        """
        self.eval()
        device = input_ids.device
        batch = input_ids.shape[0]
        
        # Track accumulated HSA context across blocks
        # This allows O(1) generation per token
        past_contexts = [None] * self.config.num_layers
...
# Utility: Create model from config dict
