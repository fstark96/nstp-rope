...
class HSAEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Projection from model dim to HSA dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.binary:
            ...
            h_binary[h_binary == 0] = 1.0  # Break ties to +1
...
class HSAContextAccumulator(nn.Module):
    ...
    def forward(
        ...
        if self.bind_mode == "xor":
            # Vectorized: shift each token by its position
            # This is O(seq_len * hsa_dim) but fully parallelizable
            ...
            for pos in unique_pos:
                mask = (positions == pos)
                if mask.any():
                    # Apply cyclic shift to tokens at this position
                    shifted = cyclic_shift(h[mask], pos.item())
                    bound[mask] = shifted
        else:
...
class HyperdimensionalAttention(nn.Module):
    ...
    def __init__(
        ...
        d_model: int,
        hsa_dim: int = 16384,
        num_heads: int = 8,  # For multi-head HSA (parallel independent contexts)
        binary: bool = True,
        bind_mode: str = "xor",
        ...
        self.encoders = nn.ModuleList([
            HSAEncoder(d_model, self.head_dim, binary=binary)
